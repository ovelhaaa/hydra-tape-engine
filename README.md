...
- leitura de ranges runtime por instância via `hydra_dsp_get_param_spec_for_handle(...)` (ex.: `delayTimeMs.max` depende de `max_delay_ms` do handle).
...
## Hardware físico (ESP32-S3)

As instruções de integração física (requisitos, pinout e comandos serial CLI) foram preservadas em:
