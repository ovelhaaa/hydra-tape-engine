...
- Triggers: `pull_request`, push on `main`, and manual `workflow_dispatch`.
- On `pull_request`: the workflow builds the web target and uploads `build-web/web` as a preview artifact (PR preview), but does **not** deploy to GitHub Pages.
- On push to `main` and `workflow_dispatch`: it builds and publishes `build-web/web` to GitHub Pages.
