...
static const hydra_dsp_param_spec kParamSpecs[HYDRA_DSP_PARAM_COUNT] = {
    {HYDRA_DSP_PARAM_FLUTTER_DEPTH, "flutterDepth", 0.0f, 100.0f, 20.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_WOW_DEPTH, "wowDepth", 0.0f, 100.0f, 15.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_DROPOUT, "dropoutSeverity", 0.0f, 100.0f, 8.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_DRIVE, "drive", 0.0f, 100.0f, 40.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_NOISE, "noise", 0.0f, 100.0f, 30.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_TAPE_SPEED, "tapeSpeed", 0.0f, 100.0f, 50.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_TAPE_AGE, "tapeAge", 0.0f, 100.0f, 40.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_HEAD_BUMP, "headBumpAmount", 0.0f, 100.0f, 30.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_AZIMUTH, "azimuthError", 0.0f, 100.0f, 10.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_FLUTTER_RATE, "flutterRate", 0.1f, 20.0f, 6.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_WOW_RATE, "wowRate", 0.1f, 5.0f, 0.8f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_DELAY_ACTIVE, "delayActive", 0.0f, 1.0f, 0.0f, HYDRA_DSP_SMOOTHING_NONE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    ...
    {HYDRA_DSP_PARAM_GUITAR_FOCUS, "guitarFocus", 0.0f, 1.0f, 0.0f, HYDRA_DSP_SMOOTHING_NONE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_TONE, "tone", 0.0f, 100.0f, 50.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_PING_PONG, "pingPong", 0.0f, 1.0f, 0.0f, HYDRA_DSP_SMOOTHING_NONE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_FREEZE, "freeze", 0.0f, 1.0f, 0.0f, HYDRA_DSP_SMOOTHING_NONE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_REVERSE, "reverse", 0.0f, 1.0f, 0.0f, HYDRA_DSP_SMOOTHING_NONE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_REVERSE_SMEAR, "reverseSmear", 0.0f, 1.0f, 0.0f, HYDRA_DSP_SMOOTHING_NONE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_SPRING, "spring", 0.0f, 1.0f, 0.0f, HYDRA_DSP_SMOOTHING_NONE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_SPRING_DECAY, "springDecay", 0.0f, 100.0f, 60.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_SPRING_DAMPING, "springDamping", 0.0f, 100.0f, 45.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_SPRING_MIX, "springMix", 0.0f, 100.0f, 50.0f, HYDRA_DSP_SMOOTHING_BLOCK_EDGE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
    {HYDRA_DSP_PARAM_RESET, "reset", 0.0f, 1.0f, 0.0f, HYDRA_DSP_SMOOTHING_NONE, HYDRA_DSP_PARAM_SPEC_FLAG_NONE},
};

static hydra_dsp_param_spec resolve_runtime_spec(const hydra_dsp_handle* handle, hydra_dsp_param_id param_id) {
  hydra_dsp_param_spec spec = kParamSpecs[param_id];
  if (handle && param_id == HYDRA_DSP_PARAM_DELAY_MS) {
    spec.max_value = std::max(spec.min_value, handle->max_delay_ms);
    spec.default_value = std::min(spec.default_value, spec.max_value);
  }
  return spec;
}
...
extern "C" {
uint32_t hydra_dsp_get_api_version(void) { return HYDRA_DSP_API_VERSION; }

uint32_t hydra_dsp_get_param_count(void) { return HYDRA_DSP_PARAM_COUNT; }

const hydra_dsp_param_spec* hydra_dsp_get_param_specs(void) { return kParamSpecs; }
...
int hydra_dsp_get_param_spec_for_handle(const hydra_dsp_handle* handle,
                                        hydra_dsp_param_id param_id,
                                        hydra_dsp_param_spec* out_spec) {
  if (!handle || !out_spec) return -1;
  if (param_id < 0 || param_id >= HYDRA_DSP_PARAM_COUNT) return -2;
  *out_spec = resolve_runtime_spec(handle, param_id);
  return 0;
}
