...
struct TapeCore::Impl {
  ...
  float inEnvL=0.f,inEnvR=0.f,envAttackCoeff=0.f,envReleaseCoeff=0.f,hissBaseGain=0.001f,hissDuckMinGain=0.25f,hissDuckStrength=2.4f;
};
...
void TapeCore::reset(){
  ...
  impl_->freezeFade=0; impl_->delayEnableRamp=0; impl_->smoothedDelaySamples=0; impl_->smoothedAzCoeff=0; impl_->springFB_L=impl_->springFB_R=0; impl_->inEnvL=impl_->inEnvR=0;
...
void TapeCore::processStereo(float inL,float inR,float* outL,float* outR){
  ...
  if(p->noise>0.001f) {
    auto updateDuck=[&](float x,float& env){
      float ax=std::fabs(x);
      float coeff=(ax>env)?impl_->envAttackCoeff:impl_->envReleaseCoeff;
      env += coeff*(ax-env);
      return clampf(1.f-(impl_->hissDuckStrength*env),impl_->hissDuckMinGain,1.f);
    };
    float noiseMult = (p->noise*impl_->hissBaseGain)*(1+(2*(1-dropoutAvg)));
    float duckL=updateDuck(inL,impl_->inEnvL);
    float duckR=updateDuck(inR,impl_->inEnvR);
  ...
  auto processCh=[&](float input,float noiseInjected,float* buffer,BiquadFilter& reproHeadBumpFilter,BiquadFilter& tr,BiquadFilter& outputGapLossFilter,AllpassFilter& az,DCBlocker& dc,TapeMagnetics& mag,BiquadFilter&iHP,BiquadFilter&iLP,OnePoleLP&feedbackGapLossFilter,BiquadFilter&fbHPF,BiquadFilter&feedbackHeadBumpFilter,AllpassFilter&feedbackPhaseFilter,OnePoleLP&dropoutFilter,DropoutFrame drop,float mod){
    ...
    // --- playback head EQ ---
    float signalForFeedback=tapeSig + noiseInjected;
