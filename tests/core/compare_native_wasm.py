...
def main():
    ...
    if args.diff_csv:
        ...
        with diff_csv_path.open('w', encoding='utf-8', newline='') as fp:
            writer = csv.writer(fp)
            writer.writerow(['sample', 'delta_l', 'delta_r', 'abs_delta_l', 'abs_delta_r'])
            for idx, dl, dr, abs_l, abs_r in diffs:
                writer.writerow([idx, f'{dl:.12e}', f'{dr:.12e}', f'{abs_l:.12e}', f'{abs_r:.12e}'])
    ...
    if args.render_wav_dir:
        ...
        write_stereo_wav(wav_dir / 'diff.wav', ((nl - wl, nr - wr) for (nl, nr), (wl, wr) in zip(native, wasm, strict=True)))
